...
function BufferForward()
	...
	for _, i in pairs(vim.fn.getbufinfo()) do
		...
		if cond1 and cond2 and cond3 and cond5 and cond6 then
...
function BufferBackward()
	...
	for _, i in pairs(bufinfo) do
		...
		if curbuf == buffirst  and cond2 and cond5 and cond5 and cond6 then
			print('<-- N: ' .. somename .. ' #: ' .. somenum)
			vim.cmd("buffer" .. somenum)
			break
...
vim.keymap.set("n", "<leader>b", function() NavBackward() end)
vim.keymap.set("n", "<leader>n", function() NavForward() end)
...
-- for lsp
-- lsp mappings are in /after/plugin/lsp.lua
